...
const PAYMENT_RETRY_KEY = "__ttm_helper_payment_retries__";
...
function getActiveZoneIndex(zoneValue: string) {
  ...
  try {
    ...
    return Number.isFinite(index) && index >= 0 && index < zoneCandidates.length ? index : 0;
  } catch {
    return 0;
  }
}
...
function zoneMatchesToken(haystack: string, zoneCode: string): boolean {
  if (!haystack || !zoneCode) return false;
  const normHaystack = compactText(haystack);
  const normZone = compactText(zoneCode);
  if (!normHaystack || !normZone) return false;

  if (normHaystack === normZone) return true;

  const escaped = normZone.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const regex = new RegExp(`(?:^|[^a-z0-9\u0E00-\u0E7F])${escaped}(?:[^a-z0-9\u0E00-\u0E7F]|$)`, "i");
  return regex.test(haystack);
}

function isZoneElementSoldOut(element: Element): boolean {
  ...
  return (
    ...
    text.includes("โซนนี้เต็ม") ||
    text.includes("เต็มแล้ว") ||
    text.includes("หมดแล้ว")
  );
}
...
function collectZoneMarkers() {
  ...
  return textNodes
    ...
    .map((node) => {
      ...
      if (!label || label.length > 20 || label === "STAGE" || label === "EXIT" || label === "เวที") {
        return null;
      }

      const containerGroup = node.closest("a, g[id], g[onclick], [role='button']");
      const nearestShape = containerGroup ? { target: containerGroup, distanceScore: 0 } : findNearestShapeTarget(node, shapeNodes);
...
function collectExactTextZoneMarkers(zone: string) {
  ...
  return candidateNodes
    ...
    .map((node) => {
      ...
      if (label !== zone && !compactText(label).includes(compactText(zone))) {
        return null;
      }

      const container = node.closest("a, button, [onclick], [role='button'], g, area");
      const nearestShape = container ? { target: container, distanceScore: 0 } : findNearestShapeTarget(node, shapeNodes);
      const target =
        nearestShape?.target ??
        container ??
        node;
...
function findZoneTarget(zone: string, logs: string[]) {
  ...
  // 1. Direct attribute match on interactive elements (SVG g, path, polygon, a, area, button)
  const interactiveCandidates = Array.from(
    document.querySelectorAll<Element>(
      "area, svg g, svg a, svg path, svg polygon, svg rect, svg circle, a, button, [role='button'], [onclick], [data-zone], [data-section], [data-name]"
    )
  );
  ...
  for (const node of interactiveCandidates) {
    ...
    if (zoneMatchesToken(attrs, normalizedZone)) {
      logs.push(`C0: attribute match -> ${normalizedZone} (${node.tagName.toLowerCase()})`);
      return node.closest("[onclick], a, area, button, [role='button'], g") ?? node;
    }
  }

  // 2. SVG text elements and their parent groups or nearest shapes
  ...
  // 3. Exact text match in clickable elements (buttons, links, labels, cards)
  ...
  // 4. Area tags with popup inspection or href inspection
  for (const area of Array.from(document.querySelectorAll<Element>("map area"))) {
    const hint = getAreaZoneHint(area);
    if (zoneMatchesToken(hint, normalizedZone)) {
      logs.push(`C0: popup/area hint match -> ${normalizedZone} (${describeArea(area)})`);
      return area;
    }
  }

  // 5. Text match with common prefixes (e.g. "Zone LM5", "โซน LM5", "LM5 (2,500)")
  const prefixCandidates = [`Zone ${zone}`, `โซน ${zone}`, `Zone${zone}`, `โซน${zone}`];
  const prefixClickable = scanClickableByTexts(prefixCandidates, false)[0]?.target ?? null;
  if (prefixClickable) {
    logs.push(`C0: clickable prefix match -> ${normalizedZone}`);
    return prefixClickable;
  }

  // 6. Check for select dropdown
  const select = document.querySelector<HTMLSelectElement>("select[name*='zone'], select[id*='zone']");
  if (select) {
    const option = Array.from(select.options).find((opt) => zoneMatchesToken(opt.text + " " + opt.value, normalizedZone));
    if (option) {
      select.value = option.value;
      select.dispatchEvent(new Event("change", { bubbles: true }));
      logs.push(`C0: zone dropdown selected -> ${normalizedZone}`);
      const submitBtn = select.closest("form")?.querySelector<HTMLElement>("button[type='submit'], input[type='submit']");
      return submitBtn ?? select;
    }
  }

  logs.push(`C0: available zone markers -> ${zoneMarkers.slice(0, 15).map((m) => m.label).join(", ")}`);
...
function readBookingSummarySeats(): string {
  const containers = Array.from(document.querySelectorAll<HTMLElement>("div, section, aside"))
    .filter((node) => visible(node))
    .filter((node) => {
      const text = normalizeText(node.textContent ?? "");
      return (text.includes("รายละเอียดการจอง") || text.includes("Booking Detail") || text.includes("เลขที่นั่ง")) && text.length < 2500;
    });
  ...
  for (const root of searchRoots) {
    ...
    // Check for seat numbers line: e.g. "เลขที่นั่ง M-20", "เลขที่นั่ง: M-20, M-21", "เลขที่นั่ง I-13"
    const seatMatch = text.match(/(?:เลขที่นั่ง|seat\s*no\.?)\s*[:\s]*([a-z0-9\s,\-]+?)(?=\s*(?:ยืนยัน|ราคา|สถานะ|โซน|รอบ|จำนวน|booking|$))/i);
    if (seatMatch?.[1]) {
      const seatVal = seatMatch[1].trim();
      if (seatVal === "-" || seatVal === " - " || seatVal === "--") {
        // Also check if ticket count > 0
        const countMatch = text.match(/(?:จำนวนที่นั่ง|ticket\s*count|quantity)\s*[:\s]*([0-9]+)/i);
        if (countMatch?.[1] && parseInt(countMatch[1], 10) > 0) {
          return `count:${countMatch[1]}`;
        }
        return "-";
      }
      if (/[a-z0-9]/i.test(seatVal)) {
        return seatVal.toUpperCase();
      }
    }

    // Fallback: check count
    const countLine = text.match(/(?:จำนวนที่นั่ง|ticket\s*count|quantity)\s*[:\s]*([0-9]+)/i);
    if (countLine?.[1] && parseInt(countLine[1], 10) > 0 && !text.includes("เลขที่นั่ง -")) {
      return `count:${countLine[1]}`;
    }

    if (text.includes("เลขที่นั่ง -") || text.includes("เลขที่นั่ง-") || text.includes("seat no. -")) {
      return "-";
    }
  }
...
function parseTargetPrice(price: string | number | null | undefined): number | null {
  if (price === null || price === undefined || price === "") return null;
  const num = typeof price === "number" ? price : parseInt(String(price).replace(/[^0-9]/g, ""), 10);
  return Number.isFinite(num) && num > 0 ? num : null;
}

const WRONG_ROWS_KEY = "__ttm_helper_wrong_rows__";

function readWrongRows(zone: string): Set<string> {
  try {
    const raw = safeSessionGet(`${WRONG_ROWS_KEY}_${zone}`);
    if (raw) return new Set(JSON.parse(raw));
  } catch { }
  return new Set();
}

function addWrongRow(zone: string, row: string, logs: string[]) {
  if (!row) return;
  const rows = readWrongRows(zone);
  rows.add(row.toUpperCase());
  safeSessionSet(`${WRONG_ROWS_KEY}_${zone}`, JSON.stringify(Array.from(rows)));
  logs.push(`S1: row ${row} marked as wrong price and skipped`);
}

function readBookingSummaryPrice(): number | null {
  const summaryContainers = Array.from(
    document.querySelectorAll("div, aside, section, table")
  ).filter((el) => {
    const t = el.textContent ?? "";
    return (t.includes("รายละเอียดการจอง") || t.includes("Booking Detail")) && t.length < 2000 && visible(el);
  });

  const searchRoots = summaryContainers.length > 0 ? summaryContainers : [document.body];

  for (const root of searchRoots) {
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT);
    while (walker.nextNode()) {
      const raw = walker.currentNode.textContent?.trim() ?? "";
      if (raw.includes("ราคาบัตร") || raw.includes("Ticket Price")) {
        const parent = walker.currentNode.parentElement;
        if (!parent || !visible(parent)) continue;

        const row = parent.closest("tr, li, div.row, div") ?? parent;
        const rowText = normalizeText(row.textContent ?? "");
        if (rowText.includes("ราคาบัตร -") || rowText.includes("ราคาบัตร-") || rowText.includes("ticket price -")) {
          return null;
        }

        const match = rowText.match(/(?:ราคาบัตร|ticket\s*price)\s*[:\s]*([0-9,]+)/i);
        if (match?.[1]) {
        ...
        const nextSibling = parent.nextElementSibling ?? row.nextElementSibling;
        if (nextSibling && visible(nextSibling)) {
          const sibText = normalizeText(nextSibling.textContent ?? "");
          if (sibText === "-" || sibText.includes("-")) continue;
          const sibMatch = sibText.match(/([1-9][0-9]{0,2}(?:,[0-9]{3})+|[1-9][0-9]{2,5})/);
          if (sibMatch?.[1]) {
            const parsed = parseInt(sibMatch[1].replace(/,/g, ""), 10);
            if (Number.isFinite(parsed) && parsed >= 100 && parsed <= 100000) {
              return parsed;
            }
          }
...
function parseRgb(colorStr: string): [number, number, number] | null {
  if (!colorStr || colorStr === "none" || colorStr === "transparent" || colorStr === "rgba(0, 0, 0, 0)") {
    return null;
  }
...
function isColorClose(c1: string, c2: string, threshold = 35): boolean {
  const rgb1 = parseRgb(c1);
  const rgb2 = parseRgb(c2);
  if (!rgb1 || !rgb2) return false;
  const dist = Math.hypot(rgb1[0] - rgb2[0], rgb1[1] - rgb2[1], rgb1[2] - rgb2[2]);
  return dist <= threshold;
}

function getElementVisualColor(node: Element): string {
  // 1. Direct SVG fill attribute
  const directFill = node.getAttribute("fill");
  if (directFill && directFill !== "none") {
    const norm = normalizeColor(directFill);
    if (norm) return norm;
  }
  // 2. Inline style fill or backgroundColor
  if (node instanceof HTMLElement || node instanceof SVGElement) {
    if (node.style.fill && node.style.fill !== "none") {
      const norm = normalizeColor(node.style.fill);
      if (norm) return norm;
    }
    if (node.style.backgroundColor && node.style.backgroundColor !== "transparent") {
      const norm = normalizeColor(node.style.backgroundColor);
      if (norm) return norm;
    }
  }
  // 3. Computed style fill (for SVG) or backgroundColor (for HTML)
  try {
    const comp = window.getComputedStyle(node);
    if (node instanceof SVGElement || ["circle", "rect", "path", "ellipse", "polygon"].includes(node.tagName.toLowerCase())) {
      if (comp.fill && comp.fill !== "none" && comp.fill !== "rgba(0, 0, 0, 0)") {
        const norm = normalizeColor(comp.fill);
        if (norm) return norm;
      }
    }
    if (comp.backgroundColor && comp.backgroundColor !== "transparent" && comp.backgroundColor !== "rgba(0, 0, 0, 0)") {
      const norm = normalizeColor(comp.backgroundColor);
      if (norm) return norm;
    }
  } catch {}
  // 4. Check parent fill attribute
  const parentFill = node.closest("[fill]");
  if (parentFill && parentFill !== node) {
    const pFill = parentFill.getAttribute("fill");
    if (pFill && pFill !== "none") {
      const norm = normalizeColor(pFill);
      if (norm) return norm;
    }
  }
  return "";
...
function findLegendColorForPrice(targetPrice: number): string {
  ...
  while (walker.nextNode()) {
    ...
    if (txt === formatted || txt === String(targetPrice) || (txt.includes(formatted) && txt.length < 20)) {
      const parent = walker.currentNode.parentElement;
      if (parent && visible(parent) && !parent.closest("aside, table, .table-summary, .booking-summary, header, footer")) {
        targetTextNode = walker.currentNode as Text;
        break;
      }
    }
  }

  if (!targetTextNode || !targetTextNode.parentElement) return "";

  const textRect = targetTextNode.parentElement.getBoundingClientRect();
  const searchRoot = targetTextNode.parentElement.closest("div, ul, section, svg, p") ?? document.body;
  const potentialIcons = Array.from(
    searchRoot.querySelectorAll("svg circle, svg rect, svg path, [fill], span, i, div, em, b")
  ).filter((el) => visible(el));

  let closestColor = "";
  let minDistance = 120;

  for (const shape of potentialIcons) {
    if (shape === targetTextNode.parentElement || shape.contains(targetTextNode)) continue;
    const rect = shape.getBoundingClientRect();
    if (rect.width < 3 || rect.height < 3 || rect.width > 50 || rect.height > 50) continue;
    const cx = rect.left + rect.width / 2;
    const cy = rect.top + rect.height / 2;
    const dist = Math.hypot(cx - textRect.left, cy - (textRect.top + textRect.height / 2));
    if (dist < minDistance) {
      const color = getElementVisualColor(shape);
      if (color && !isColorClose(color, "rgb(255, 255, 255)", 15) && !isColorClose(color, "rgb(0, 0, 0)", 15)) {
        minDistance = dist;
        closestColor = color;
      }
    }
...
function hasLegendForPrice(targetPrice: number): boolean {
  return Boolean(findLegendColorForPrice(targetPrice));
}

function deselectCurrentlySelectedSeats(logs: string[]) {
  // 1. Deselect using candidate list matching sidebar seat names (e.g. M-20 -> row M, seat 20)
  const seatsStr = readBookingSummarySeats();
  if (seatsStr && seatsStr !== "-") {
    const rawTokens = seatsStr.split(",").map((s) => s.trim().toUpperCase());
    const candidates = collectSeatCandidates(logs);

    for (const token of rawTokens) {
      const rowMatch = token.match(/^([A-Z]{1,2})\s*[- ]?\s*(\d{1,3})$/);
      if (rowMatch) {
        const row = rowMatch[1];
        const num = parseInt(rowMatch[2], 10);
        const matchCandidate = candidates.find((c) => c.rowLabel.toUpperCase() === row && c.seatNumber === num);
        if (matchCandidate) {
          logs.push(`S0: deselecting seat ${row}-${num} on map via candidate`);
          clickSeatCandidate(matchCandidate, logs);
          continue;
        }
      }

      // Fallback: search all clickable seat nodes by text
      const compact = token.replace(/[^A-Z0-9]/g, "");
      const allSeatElements = Array.from(document.querySelectorAll("svg circle, svg rect, svg [data-seat], svg g, [onclick]"));
      const match = allSeatElements.find((node) => {
        const txt = elementSearchText(node).toUpperCase().replace(/[^A-Z0-9]/g, "");
        return txt.includes(compact);
      });
      if (match) {
        logs.push(`S0: deselecting seat ${token} on map via element search`);
        clickElement(match);
      }
    }
  }

  // 2. Also deselect any elements with checkmark or selected class/indicator
  const selectedElements = Array.from(
    document.querySelectorAll(
      "svg circle[fill*='check'], svg image, svg g.selected, svg [class*='selected'], [class*='selected'], circle[data-selected='true'], [aria-selected='true']"
    )
  ).filter((el) => visible(el));

  for (const el of selectedElements) {
    logs.push("S0: deselecting marked seat element...");
    clickElement(el);
  }
}

function getSeatCandidatePrice(node: Element): number | null {
  const dataPrice = node.getAttribute("data-price") || node.getAttribute("price");
  if (dataPrice) {
    const p = parseInt(dataPrice.replace(/,/g, ""), 10);
  ...
  const match = text.match(/(?:ราคา|price|thb|baht)?\s*([1-9][0-9]{2,5})\s*(?:บาท|thb)?/i);
  if (match?.[1]) {
...
function hasSeatWarningModal() {
  ...
  return dialogCandidates.some((node) => {
    ...
    return (
      ...
      text.includes("โซนนี้เต็ม") ||
      text.includes("sold out") ||
      text.includes("ไม่ว่าง") ||
      text.includes("เต็มแล้ว") ||
      text.includes("บัตรหมด")
    );
  });
}
...
function goBackToZoneSelection(logs: string[]) {
  const button =
    findClickableByTexts(["เลือกโซนอื่น", "back to zone", "change zone", "เลือกโซน"]) ||
    document.querySelector<HTMLElement>("a[href*='zones.php'], button[onclick*='zones.php'], .btn-back-zone, #btn-back");
  const clicked = Boolean(button && (clickElement(button) || clickAtCenter(button)));
  if (!clicked) {
    window.history.back();
    logs.push("Z1: back to zone selection via history.back()");
    return true;
  }
  logs.push("Z1: back to zone selection clicked");
  return clicked;
}
...
function resolveDeliveryValue(value: string) {
  ...
  if (normalized === "pickup" || normalized.includes("ตนเอง") || normalized.includes("ตัวเอง")) {
    return "รับบัตรด้วยตนเอง";
  }

  if (normalized === "postal" || normalized === "mail" || normalized === "delivery" || normalized.includes("ไปรษณีย์")) {
    return "จัดส่งทางไปรษณีย์";
  }
...
function chooseZone(zoneValue: string, logs: string[]) {
  ...
  if (hasSeatWarningModal()) {
    ...
    if (
      warningText.includes("เต็ม") ||
      warningText.includes("sold out") ||
      warningText.includes("unavailable") ||
      warningText.includes("หมด") ||
      warningText.includes("ไม่ว่าง")
    ) {
      const nextZone = advanceToNextZone(zoneValue, logs, "zone-modal-sold-out");
      if (nextZone) {
        logs.push(`C1: zone modal indicated sold out -> advancing to ${nextZone}`);
      }
    }
  }
  ...
  if (recentlyMarked(ZONE_MARKER_KEY, activeZone, 1500)) {
    logs.push(`C1: waiting after zone click -> ${activeZone}`);
    return { status: "waiting" as const, zone: activeZone };
  }

  const target = findZoneTarget(activeZone, logs);

  if (target) {
    if (isZoneElementSoldOut(target)) {
      logs.push(`C1: zone ${activeZone} is marked sold out on map`);
      const nextZone = advanceToNextZone(zoneValue, logs, "map-sold-out");
      if (nextZone) {
        return { status: "waiting" as const, zone: nextZone };
      }
      return { status: "missing" as const, zone: activeZone };
    }

    if (target.tagName === "AREA") {
      logs.push(`C1: click area target -> ${activeZone} (${describeArea(target)})`);
    } else {
      logs.push(`C1: click target tag -> ${activeZone} (${target.tagName.toLowerCase()})`);
    }

    const clicked = clickElement(target) || clickAtCenter(target);
    if (clicked) {
      writeMarker(ZONE_MARKER_KEY, activeZone, 1);
      clearMarker(SEAT_MARKER_KEY);
      clearSkippedSeats(activeZone);
      logs.push(`C1: zone clicked -> ${activeZone}`);
      return { status: "clicked" as const, zone: activeZone };
    }

    logs.push(`C1: zone click failed -> ${activeZone}`);
    return { status: "waiting" as const, zone: activeZone };
  }

  logs.push(`C1: zone ${activeZone} not found yet on map (waiting for map to render)`);
  return { status: "missing" as const, zone: activeZone };
}
...
function chooseSeats(preset: TtmPresetMessage, logs: string[]) {
  ...
  const targetPrice = parseTargetPrice(preset.price);

  if (targetPrice) {
    const currentSeats = readBookingSummarySeats();
    const initialSummaryPrice = readBookingSummaryPrice();

    // 1. If NO seats are selected yet, but a price is already shown (single-price zone that doesn't match target)
    if ((!currentSeats || currentSeats === "-") && initialSummaryPrice && initialSummaryPrice !== targetPrice) {
      const targetAvailableInLegend = hasLegendForPrice(targetPrice);
      if (!targetAvailableInLegend) {
        logs.push(`S0: zone fixed price (${initialSummaryPrice}) does not match target price (${targetPrice})`);
        deselectCurrentlySelectedSeats(logs);
        clearMarker(SEAT_MARKER_KEY);
        const nextZone = advanceToNextZone(zoneValue, logs, `price-mismatch-${initialSummaryPrice}-vs-${targetPrice}`);
        if (nextZone) {
        ...
        return {
          status: "price-mismatch" as const,
          selected: [] as SeatCandidate[],
          requestedCount,
          strategy,
          confirmed: false,
          nextZone: undefined,
          currentPrice: initialSummaryPrice,
          targetPrice
        };
      } else {
        logs.push(`S0: zone default price is ${initialSummaryPrice}, but legend has target price ${targetPrice} -> proceeding to select seats`);
      }
    }

    // 2. If seats ARE already selected, but their price does NOT match targetPrice:
    if (currentSeats && currentSeats !== "-" && initialSummaryPrice && initialSummaryPrice !== targetPrice) {
      logs.push(`S0: currently selected seats (${currentSeats}) price (${initialSummaryPrice}) != target (${targetPrice}) -> deselecting`);
      deselectCurrentlySelectedSeats(logs);
      clearMarker(SEAT_MARKER_KEY);
      const rawLabels = currentSeats.split(",").map((s) => s.trim().toUpperCase());
      for (const token of rawLabels) {
        const rowMatch = token.match(/^([A-Z]{1,2})/);
        const row = rowMatch ? rowMatch[1] : token.replace(/[^A-Z]/g, "");
        if (row) {
          addWrongRow(zone, row, logs);
        }
      }
      return {
  ...
  if (existingMarker?.phase === "selected") {
    if (targetPrice) {
      const currentPrice = readBookingSummaryPrice();
      if (currentPrice && currentPrice !== targetPrice) {
        logs.push(`S3: selected seats price (${currentPrice}) does not match target (${targetPrice}) -> deselecting and skipping row`);
        deselectCurrentlySelectedSeats(logs);
        clearMarker(SEAT_MARKER_KEY);
        for (const seatStr of (existingMarker.seats ?? [])) {
          const rowMatch = seatStr.toUpperCase().match(/^([A-Z]{1,2})/);
          const row = rowMatch ? rowMatch[1] : seatStr.replace(/[^A-Z]/g, "");
          if (row) {
            addWrongRow(zone, row, logs);
          }
          addSkippedSeats(zone, [seatStr], logs);
        }
        return {
          status: "retry" as const,
          selected: [] as SeatCandidate[],
    ...
    const autoConfirm = preset.autoConfirmSeat ?? preset.auto_confirm_seat ?? false;

    if (!autoConfirm) {
      logs.push(`S3: seats selected -> ${bookingSummarySeats} (holding for user, auto-confirm disabled)`);
      return {
  ...
  const candidates = collectSeatCandidates(logs);
  const wrongRows = targetPrice ? readWrongRows(zone) : new Set<string>();

  const eligibleCandidates = candidates.filter((candidate) => {
    if (skippedSeats.has(`${candidate.rowLabel}${candidate.seatNumber}`.toUpperCase())) {
      return false;
    }
    if (wrongRows.has(candidate.rowLabel.toUpperCase())) {
      return false;
    }
    const explicitPrice = getSeatCandidatePrice(candidate.element);
    if (targetPrice && explicitPrice !== null && explicitPrice !== targetPrice) {
      return false;
    }
    return true;
  });

  let filteredCandidates = eligibleCandidates;
  if (targetPrice) {
    const targetColor = findLegendColorForPrice(targetPrice);
    if (targetColor) {
      const colorMatches = eligibleCandidates.filter((c) => {
        const fill = getElementVisualColor(c.element);
        return fill && isColorClose(fill, targetColor);
      });
      if (colorMatches.length >= requestedCount) {
        logs.push(`S0: prioritized ${colorMatches.length} seats matching price ${targetPrice} color (${targetColor})`);
        filteredCandidates = colorMatches;
      } else {
        logs.push(`S0: found only ${colorMatches.length} seats matching price ${targetPrice} color (${targetColor}), need ${requestedCount}`);
      }
    }
  }
  ...
  if (wrongRows.size) {
    logs.push(`S0: skipped wrong rows -> ${Array.from(wrongRows).join(", ")}`);
  }
  if (targetPrice) {
    logs.push(`S0: target price -> ${targetPrice} (available candidates: ${filteredCandidates.length}/${candidates.length})`);
  }

  if (!filteredCandidates.length) {
    logs.push(`S1: no seat candidates remaining in zone ${zone}${targetPrice ? ` matching price ${targetPrice}` : ""}`);
    const nextZone = advanceToNextZone(zoneValue, logs, targetPrice ? `no-seats-at-price-${targetPrice}` : "no-seats-available");
    if (nextZone) {
      goBackToZoneSelection(logs);
      return {
        status: "zone-fallback" as const,
    ...
    return {
      status: targetPrice ? ("no-seats-at-price" as const) : ("not-found" as const),
      selected: [] as SeatCandidate[],
      requestedCount,
      strategy,
      confirmed: false,
...
function isDeliverySelected(deliveryValue: string): boolean {
  const summaryTables = Array.from(document.querySelectorAll("table, .table-summary, .order-summary, .box-summary, .summary-table"));
  for (const table of summaryTables) {
    const text = normalizeText(table.textContent ?? "");
    if (text.includes("วิธีการรับบัตร") && !text.includes("วิธีการรับบัตร -") && !text.includes("วิธีการรับบัตร-")) {
      return true;
    }
  }

  const activeCards = Array.from(document.querySelectorAll(".active, .selected, .checked, [aria-selected='true']"));
  for (const card of activeCards) {
    const text = normalizeText(card.textContent ?? "");
    if (text.includes("รับบัตรด้วยตนเอง") || text.includes("จัดส่งทางไปรษณีย์") || text.includes("อี-ทิกเก็ต")) {
      return true;
    }
  }
...
function chooseDeliveryMethod(deliveryValue: string, logs: string[]) {
  const aliasMap: Record<string, string[]> = {
    pickup: ["รับบัตรด้วยตนเอง", "รับบัตรด้วยตัวเอง", "รับด้วยตนเอง", "ตนเอง", "pickup"],
    postal: ["จัดส่งทางไปรษณีย์", "ส่งไปรษณีย์", "postal", "delivery", "ไปรษณีย์"],
    "e-ticket": ["อี-ทิกเก็ต", "e-ticket", "eticket"]
  };

  const keys = aliasMap[deliveryValue] ?? [
    deliveryValue,
    "รับบัตรด้วยตนเอง",
    "รับบัตรด้วยตัวเอง",
    "ตนเอง"
  ...
  if (isDeliverySelected(deliveryValue)) {
    logs.push(`D1: delivery already active (${deliveryValue})`);
    return true;
  }
  ...
  if (target) {
    ...
    const card = target.closest<HTMLElement>(
      "label, [onclick], [role='button'], button, a, [class*='delivery'], [class*='pickup'], [class*='box'], [class*='card'], [class*='item'], .col-6, .col-sm-6, .col-xs-6, div"
    );
    if (card && card !== target) {
      const rect = card.getBoundingClientRect();
      if (rect.width > 0 && rect.width < 650 && rect.height > 0 && rect.height < 400) {
        clickElement(card);
        clickAtCenter(card);
      }
    }
    ...
    const radio =
      ...
      target.parentElement?.querySelector<HTMLInputElement>("input[type='radio']");
    if (radio) {
      if (!radio.checked) {
        radio.checked = true;
      }
    ...
    logs.push(`D1: delivery clicked -> ${deliveryValue} (${target.tagName.toLowerCase()})`);
    return true;
  }

  const deliveryHeading = Array.from(document.querySelectorAll("div, h1, h2, h3, h4, h5, span, p, label"))
    .find((el) => normalizeText(el.textContent ?? "").includes("กรุณาเลือกวิธีการรับบัตร"));
  const section = deliveryHeading?.closest("div.row, div.box, div.panel, section, form, div") ?? document.body;

  const candidateNodes = Array.from(section.querySelectorAll<HTMLElement>("div, label, a, button, span, p, input"));
  for (const node of candidateNodes) {
    if (!visible(node)) continue;
    const txt = normalizeText(node.textContent ?? "");
    if (keys.some((k) => txt === normalizeText(k) || (txt.includes(normalizeText(k)) && txt.length < 50))) {
      clickElement(node);
      clickAtCenter(node);
      const radio = node.querySelector<HTMLInputElement>("input[type='radio']") ?? node.parentElement?.querySelector<HTMLInputElement>("input[type='radio']");
      if (radio) {
        radio.checked = true;
        try { radio.click(); } catch { }
        radio.dispatchEvent(new Event("change", { bubbles: true }));
      }
      logs.push(`D1: delivery clicked via section scan -> ${deliveryValue}`);
      return true;
    }
  }

  logs.push(deliveryValue ? `D1: delivery not found -> ${deliveryValue}` : "D1: no delivery configured");
  return false;
}

function isPaymentSelected(paymentValue: string): boolean {
  const summaryTables = Array.from(document.querySelectorAll("table, .table-summary, .order-summary, .box-summary, .summary-table"));
  for (const table of summaryTables) {
    const text = normalizeText(table.textContent ?? "");
    if (text.includes("วิธีการชำระเงิน") && !text.includes("วิธีการชำระเงิน -") && !text.includes("วิธีการชำระเงิน-")) {
      return true;
    }
  }
  return false;
...
function choosePayment(paymentValue: string, logs: string[]) {
  ...
  if (isPaymentSelected(paymentValue)) {
    logs.push(`D1: payment already active (${paymentValue})`);
    return true;
  }
  ...
  if (target) {
    ...
    const card = target.closest<HTMLElement>(
      "label, [onclick], [role='button'], button, a, [class*='payment'], [class*='box'], [class*='card'], [class*='item'], .col-6, .col-sm-6, .col-xs-6, div"
    );
    if (card && card !== target) {
      const rect = card.getBoundingClientRect();
      if (rect.width > 0 && rect.width < 650 && rect.height > 0 && rect.height < 400) {
...
export function runTtmPreset(preset: TtmPresetMessage): TtmRunReport {
  ...
  if (state !== "payment") {
    safeSessionRemove(PAYMENT_RETRY_KEY);
  }
  ...
  switch (state) {
        ...
        detail:
          zoneResult.status === "clicked"
            ...
            : zoneResult.status === "waiting"
              ...
              : zoneValue.trim()
                ? "A: อยู่หน้าเลือกโซน แต่หาโซนที่ตั้งไว้ไม่เจอ รอให้ผู้ใช้กดเข้าผังเอง"
                : "A: อยู่หน้าเลือกโซน (ไม่ได้ระบุโซน) รอให้ผู้ใช้กดเลือกโซนเองบนหน้าเว็บ",
        state,
        step: "A",
        stopped: false,
    ...
    case "seats": {
      ...
      if (seatResult.status === "held") {
        const seatsText = (seatResult as any).summary || selectedLabels || "เรียบร้อย";
        return {
          action: "seat-held",
          detail: `B: ล็อคที่นั่ง (${seatsText}) แล้ว (หยุดรอเพื่อให้ผู้ใช้ตรวจหรือเลือกใหม่)`,
          state,
          step: "B",
          stopped: true,
          matched: seatsText,
      ...
      if (seatResult.status === "price-mismatch") {
        return {
          action: "seat-price-mismatch",
          detail: `B: ราคาบัตรในโซนนี้ (${seatResult.currentPrice.toLocaleString()} บาท) ไม่ตรงกับราคาที่ตั้งไว้ (${seatResult.targetPrice.toLocaleString()} บาท) บอทจึงหยุดเพื่อความปลอดภัย`,
          state,
          step: "B",
          stopped: true,
          matched: null,
      ...
      if (seatResult.status === "no-seats-at-price") {
        return {
          action: "seat-price-not-found",
          detail: `B: ไม่พบที่นั่งว่างในราคาที่ตั้งไว้ (${seatResult.targetPrice.toLocaleString()} บาท)`,
          state,
          step: "B",
          stopped: true,
          matched: null,
    ...
    case "payment": {
      const deliveryMethod = preset.deliveryMethod ?? preset.delivery ?? "pickup";
      const deliveryDone = chooseDeliveryMethod(deliveryMethod, logs);
      const paymentDone = choosePayment(paymentMethod, logs);

      const retries = Number(safeSessionGet(PAYMENT_RETRY_KEY) ?? 0);
      const allDone = (!deliveryMethod || deliveryDone) && (!paymentMethod || paymentDone);

      if (allDone || retries >= 10) {
        safeSessionRemove(PAYMENT_RETRY_KEY);
        return {
          action: "payment-reached",
          detail: `D: ถึงหน้าชำระเงินแล้ว (รับบัตร: ${deliveryDone ? "เลือกแล้ว" : "ยังไม่ได้เลือก"}, จ่ายเงิน: ${paymentDone ? "เลือกแล้ว" : "ยังไม่ได้เลือก"})`,
          state,
          step: "D",
          stopped: true,
      ...
      safeSessionSet(PAYMENT_RETRY_KEY, String(retries + 1));
      return {
        action: "payment-retrying",
        detail: `D: ถึงหน้าชำระเงินแล้ว กำลังเลือกวิธีรับบัตรและชำระเงิน (${retries + 1}/10)...`,
        state,
        step: "D",
        stopped: false,
        matched: paymentMethod || null,
