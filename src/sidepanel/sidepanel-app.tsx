...
function normalizeDraft(savedDraft?: Partial<HelperDraft> | null): HelperDraft {
  ...
  const merged = {
    ...
    names: Array.isArray(savedDraft.names) ? savedDraft.names.map(String) : baseDraft.names,
    backup_zones: savedDraft.backup_zones ?? ""
  };
  return merged;
}
...
export function SidePanelApp() {
  ...
  useEffect(() => {
    async function boot() {
      ...
      const savedDraft = await storage.getDraft<HelperDraft>("helper_draft");
  ...
  async function runHelper() {
    ...
    try {
      ...
      const combinedZones = [draft.zones.trim(), draft.backup_zones?.trim()]
        .filter(Boolean)
        .join(", ");
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-brand-text font-sans">
      <header className="bg-[#1c1c1f] px-3 py-2.5 rounded-t-xl shadow-sm text-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-600 bg-white flex items-center justify-center">
            <img src="/logo.png" alt="Logo" className="w-full h-full object-cover" />
          </div>
          <div className="font-extrabold text-sm text-white leading-tight">
            {APP_CONFIG.extendtionName} <span className="text-brand-red text-[12px] ml-1 font-normal">V{APP_CONFIG.displayVersion}</span>
          </div>
        </div>
      </header>
      <div className="mx-auto flex max-w-full flex-col gap-1 px-2 mt-1">
              ...
              {section === "concert" ? (
                <div className="grid gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="รอบ" hint="ไม่บังคับ">
                      <NumberInput placeholder="เช่น 1" value={draft.round} onChange={(value) => setDraft((current) => ({ ...current, round: value }))} />
                    </Field>
                    <Field label="โซนหลัก" hint="ไม่บังคับ">
                      <TextInput placeholder="เช่น LM5" value={draft.zones} onChange={(value) => setDraft((current) => ({ ...current, zones: value }))} />
                    </Field>
                  </div>
                  <Field label="โซนสำรอง (ถ้าโซนหลักเต็ม)" hint="ไม่บังคับ เช่น LM6, LM2">
                    <TextInput
                      placeholder="เช่น LM6, LM2, LTR2"
                      value={draft.backup_zones ?? ""}
                      onChange={(value) => setDraft((current) => ({ ...current, backup_zones: value }))}
                    />
                  </Field>
                  <div className="grid grid-cols-2 gap-3">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none text-[12px] font-semibold text-gray-700 hover:text-gray-900 py-1">
                    <input
                      type="checkbox"
                      checked={draft.auto_confirm_seat ?? false}
                      onChange={(e) => setDraft((current) => ({ ...current, auto_confirm_seat: e.target.checked }))}
                      className="h-4 w-4 rounded border-gray-300 text-brand-red focus:ring-brand-red accent-brand-red"
                    />
                    <span>ยืนยันที่นั่งอัตโนมัติ (หากไม่ติ๊ก บอทจะหยุดรอเพื่อให้เลือกใหม่)</span>
                  </label>
                  <div className="grid grid-cols-2 gap-3">
                ...
                <div className="grid gap-2.5">
                  {draft.names.length === 0 ? (
                    <div className="text-center py-2 text-[12px] text-gray-400">
                      ยังไม่มีการระบุชื่อบนบัตร
                    </div>
                  ) : (
                    draft.names.map((nameValue, index) => (
                      <div key={index} className="flex items-end gap-2">
                        <div className="flex-1">
                          <Field label={`ชื่อ #${index + 1}`} hint="ไม่บังคับ">
                            <TextInput
                        ...
                        <button
                          type="button"
                          className="h-[38px] w-[38px] flex items-center justify-center rounded-xl border border-gray-200 bg-gray-50 text-gray-400 hover:text-red-500 hover:bg-red-50 hover:border-red-200 transition-colors shrink-0"
                          title="ลบชื่อนี้"
                          onClick={() =>
                            setDraft((current) => ({
                              ...current,
                              names: current.names.filter((_, i) => i !== index)
                            }))
                          }
                        >
                  ...
                  <button
                    type="button"
                    className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl border border-dashed border-gray-300 bg-gray-50/60 text-[12px] font-semibold text-gray-600 hover:bg-gray-100 hover:border-gray-400 hover:text-brand-red transition-colors mt-1"
                    onClick={() =>
                      setDraft((current) => ({
                        ...current,
                        names: [...current.names, ""]
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="วิธีการรับบัตร">
                      <SelectInput
                        ...
                        options={[
                          { value: "pickup", label: "รับบัตรด้วยตนเอง" },
        ...
        <div className="mt-2">
          <button type="button" className="w-full rounded-xl border border-gray-200 bg-white px-3 py-3 text-[12px] font-bold text-gray-700 flex justify-center items-center gap-2 hover:bg-gray-50 transition-colors" onClick={resetDraft}>
